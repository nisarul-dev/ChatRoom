...
var autoLoad = setInterval(function () {
    $.post(url, {receiver: window.receiver_id, msg_limit: window.msgLimit},
        function (result) {
            if (window.result != result) {
                $(".message-body").html(result);
                window.result = result;
                window.msgArrived = true;
                ...
                if ($(".message-body")[0].scrollTop > ($(".message-body")[0].scrollHeight - $(".message-body").height() - 240)) {
                    $(".message-body").scrollTop($(".message-body")[0].scrollHeight);
                        ...
                        // On first page load go to bottom
                        $(".message-body").scrollTop($(".message-body")[0].scrollHeight);
                }
    ...
    if ($(".message-body")[0].scrollTop == 0 && window.msgArrived == true) {
        window.msgLimit += 10;
    }
}, 500);
